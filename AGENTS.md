...
scripts/
  ...
  box-brief.md              Agent brief — loaded by Claude Code and Codex at the start of every session in every box
...
2. `build-base` runs only if `Containerfile.base`, `scripts/`, `local-bin/`, or `aur/` changed (minus the exclusions in the `base:` filter: `aur/*.md` and the scripts that never reach an image)
...
4. All images are tagged `latest` + `YYYY-MM-DDTHHMM` (e.g. `2026-03-04T2100`, UTC) and pushed to ghcr.io
...
Do not use `--bare` for the OAuth reviewer: it cannot use subscription tokens.
CLI upgrades must preserve isolation. See `aur/README.md` for token rotation,
cloud-routine retirement, human merging, deployment and recovery. These scripts
are CI-only and excluded from image path filtering. Live rules and owner review
must protect this policy; CODEOWNERS alone does not enforce review.

## Containerfile Conventions

- **`Containerfile.base` is a three-stage build**: `sysbase` (`FROM archlinux:latest`, every official-repo package), `aur-builder` (`FROM sysbase`; builds the vendored AUR packages as `builduser` and collects the `.pkg.tar.zst` files in `/out`; discarded), and the final stage (`FROM sysbase`; one `pacman -U` with the builder's `/out` bind-mounted via `RUN --mount=type=bind,from=aur-builder` — never `COPY --from`, which is a layer of its own and would ship the archives — then everything else). Image layers are additive — a delete in a later RUN does not shrink an earlier layer — so build garbage must never reach the final stage: makepkg trees, make-dependencies and `builduser` exist only in the builder; every `pacman` RUN cleans its own download cache; a `chmod -R` over a tree belongs in the RUN that created the tree (on its own it copies every file up again); `aur-builder` sets `!debug` in `makepkg.conf` so no `-debug` split packages are built. Before the split, 16.6 GB of base image carried about 8.5 GB of such residue. Only `sysbase` declares the `BUILD_DATE`/`BUILD_SHA` ARGs up front — that is what busts the layer cache nightly so `pacman -Syu` actually updates; the other stages inherit the bust via their parent layer and the final stage re-declares the ARGs only for `/etc/box-build-info`
...
- **The agent brief (`scripts/box-brief.md`) reaches Claude Code and Codex by two different routes.** Both tools load a machine-wide instruction file at the start of every session regardless of cwd, but only one of them has an `/etc` slot. Claude Code reads the managed-policy file `/etc/claude-code/CLAUDE.md`, which `Containerfile.base` writes from the brief — that slot loads *before* user (`~/.claude/CLAUDE.md`) and project `CLAUDE.md`, so a repo's own instructions still win on conflict, and it needs no per-home wiring, which means a wiped or brand-new box home still gets it. Codex has no equivalent: it reads `$CODEX_HOME/AGENTS.md` (default `~/.codex/AGENTS.md`), so `init-user.sh` symlinks that at `/usr/local/share/box-init/box-brief.md` — and **only when nothing exists at that path**, because a real file there is the user's own and clobbering it would silently drop their instructions. Keep the brief short and strictly factual about the container: it is prepended to every conversation in every box, and it costs context in all of them. It is *not* the place for personal preferences — those belong in each box's `~/.claude/CLAUDE.md`. Editing it requires a base rebuild + `box upgrade` to take effect
- **The host session bus address must be set in `shell-env.sh`, never `shell-init.sh`** — this is an ordering contract, not a preference. distrobox's own `/etc/profile.d/distrobox_profile.sh` defaults `DBUS_SESSION_BUS_ADDRESS` to `unix:path=/run/user/$UID/bus` and then calls `host-spawn` four times to import `XAUTHORITY`/`XAUTHLOCALHOSTNAME`/`WAYLAND_DISPLAY`/`DISPLAY` from the host; `host-spawn` reaches the host over that bus. In a **no-init** box (`dev`) that default is accidentally right — distrobox bind-mounts the host's `/run/user/$UID`, so the socket is there. In an **init** box (`priv`, `work`) `/run` is a fresh tmpfs and the host's runtime dir exists only under `/run/host`, so every login shell printed `dial unix /run/user/1000/bus: connect: no such file or directory` and silently lost the display variables. `shell-init.sh` set the right value but is sourced from `~/.zshrc`, which runs *after* `/etc/profile.d` — too late. `shell-env.sh` wins in both shells: `/etc/zsh/zshenv` is read before anything else by every zsh, and `box-env.sh` sorts before `distrobox_profile.sh` in `/etc/profile.d`. `PULSE_SERVER` lives beside it for the same host-socket reason (the container's own `/run/user/$UID/pulse` socket is created by distrobox-enter but connected to nothing in an init box). Note that installing `host-spawn` in the image is what turned this from a cosmetic `command not found: host-spawn` into a failing call — do not "fix" a recurrence by removing host-spawn
...
- Clean pacman's cache in the same RUN as every `pacman -S`/`-Syu`/`-U` (`&& pacman -Scc --noconfirm && rm -rf /var/cache/pacman/pkg/*`). A cleanup in a later RUN removes nothing from the image

### Adding a package

- Official, needed by all boxes: add to `Containerfile.base` `pacman -S --noconfirm --needed <pkg>`
- AUR, needed by all boxes: **never** add a `yay -S` line. Vendor and vet the PKGBUILD first (`box vendor-aur <pkgbase>`, review the diff, commit `aur/`), then add a `makepkg` build step to the `aur-builder` stage of `Containerfile.base` ending in `mv ./*.pkg.tar* /out/` — the final stage's `pacman -U /tmp/pkgs/*` installs whatever lands there. Split pkgbases follow the `google-cloud-cli` pattern (build once, `pacman -U` only the wanted outputs). See `aur/README.md`.
...
- `box vendor-aur <pkgbase>... | --all` re-clones AUR PKGBUILDs into `aur/`, prints a diff against the committed copy to vet, and updates `aur/manifest.tsv`; wraps `scripts/vendor-aur.sh`. Use it to bump a stale vendored `-bin`/`-git` package (review the diff, then commit `aur/`). This now also runs nightly in `.github/workflows/aur-bump.yml`, which opens a PR rather than committing — the manual command remains for one-off bumps. **Vetting that diff is the whole point of vendoring** — follow the step-by-step audit procedure in `aur/README.md` ("Auditing a `vendor-aur` bump") rather than eyeballing it: only validated literal version/checksum/manifest changes are routine; inspect every changed line for executable syntax
- Common workflows:
...
# Draft publication, model isolation and merge authority must remain enforced.
python3 scripts/test-aur-review.py
```
...
- The `base:` filter ends with `- '!aur/*.md'`, so editing `aur/README.md` alone does not rebuild anything. `Containerfile.base` copies `aur/` only into the throwaway `aur-builder` stage, so those docs never reach the final image. Keep the negation **after** `'aur/**'` — paths-filter applies the patterns in order
