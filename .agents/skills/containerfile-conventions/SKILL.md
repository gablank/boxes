...
- Three stages: `sysbase` (`FROM archlinux:latest`, all official-repo packages), `aur-builder` (`FROM sysbase`, discarded after the build) and the final image (`FROM sysbase`). Layers are additive — deleting in a later RUN frees nothing — so build garbage is confined to the builder or cleaned in the RUN that made it
...
- Vendoring pins recipes, not all downloaded code: Oh My Zsh follows floating Git HEAD, the gcloud component installer fetches extra artifacts, and npm/editor extensions are unpinned. Review complete PKGBUILD assignments; never strip version/checksum lines as inherently safe. See `aur/README.md`.
- Builds the AUR packages in `aur-builder` as `builduser`, collecting each pkgbase's `.pkg.tar.zst` files in `/out` (`mv ./*.pkg.tar* /out/` at the end of its RUN; google-cloud-cli moves only gcloud core + the GKE auth plugin). The final stage bind-mounts the builder's `/out` for a single `RUN --mount=type=bind,from=aur-builder,source=/out,target=/tmp/pkgs pacman -U ...`, which also pulls the runtime dependencies. Not `COPY --from`: that is a layer of its own, so the archives would ship and a later `rm` would only hide them. Sources, make-dependencies (e.g. the Go toolchain for yay) and `builduser` never reach the image
- `aur-builder` flips `debug` to `!debug` in `/etc/makepkg.conf` so no `-debug` split packages are built or installed
...
- Installs `host-spawn` to `/usr/bin/host-spawn` from a version-pinned, sha256-checked upstream GitHub release — it is in neither the official repos nor the AUR, keep the checksum pin. The npm and editor-extension installs have separate upstream trust and are not locked by this pin. distrobox requires it (`/etc/profile.d/distrobox_profile.sh` on every login shell, and `distrobox-host-exec`) but bind-mounts only its own shell scripts and no binary. Its fallback is a silent best-effort download during `distrobox-init`, into the writable layer that every `replace=true` recreate discards; it lands in `dev` but not in `priv`/`work` (suspected: no egress at init time inside their own netns, `unshare_netns = true`). **Contract:** the pinned version must be >= the `host_spawn_version` that the host's `distrobox-host-exec` requires, or that prompt returns. The binary dispatches on its own `argv[0]`, so it must keep the name `host-spawn` — a differently named symlink to it runs *that* command on the host.
- Pre-installs Cursor extensions to `/opt/cursor-extensions/`; the `chmod -R a+w` that follows lives in the same RUN — as its own RUN it copied the whole ~600 MB tree into a second layer
- COPYs the four `scripts/*.sh` init scripts to `/usr/local/share/box-init/` and `local-bin/` into the image, and hooks `shell-env.sh` into `/etc/zsh/zshenv` + `/etc/profile.d/box-env.sh`
- COPYs `scripts/box-brief.md` to `/usr/local/share/box-init/box-brief.md` and installs it as `/etc/claude-code/CLAUDE.md`, Claude Code's managed-policy file — loaded at the start of every session in the box whatever the cwd, ahead of user and project `CLAUDE.md`. Codex has no `/etc` slot, so `init-user.sh` symlinks `~/.codex/AGENTS.md` at the same copy (only when nothing is already there). Keep the brief short and strictly factual about the container: it costs context in every conversation in every box, and personal preferences belong in each box's `~/.claude/CLAUDE.md` instead. See the matching bullet in `AGENTS.md`.
- Writes `/etc/box-build-info` using `BUILD_DATE` and `BUILD_SHA` build args
- Clean pacman's cache in the same RUN as each `pacman -S`/`-Syu`/`-U` (`&& pacman -Scc --noconfirm && rm -rf /var/cache/pacman/pkg/*`); the final RUN repeats it as a backstop
...
- All boxes, AUR: **never** add a `yay -S` line. Vendor + vet the PKGBUILD first (`box vendor-aur <pkgbase>`, review the diff, commit `aur/`), then add a `makepkg` build step to the `aur-builder` stage of `Containerfile.base` ending in `mv ./*.pkg.tar* /out/` (the final stage installs everything in `/out`). Split pkgbases follow the `google-cloud-cli` pattern (build once, `pacman -U` only the wanted outputs). See `aur/README.md`.
