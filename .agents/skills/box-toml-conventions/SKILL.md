...
`box.toml` is the source of truth for each box. `scripts/compile-box-toml.py` compiles it to `distrobox.ini` (gitignored, regenerated by every command that touches config: `assemble`, `set-image`, `upgrade`, `init`). **Never edit `distrobox.ini` directly.**
...
See `.agents/skills/box-cli-conventions/SKILL.md` — CLI workflows are deliberately documented only there (plus `README.md` and `AGENTS.md`), never duplicated into other skills.
