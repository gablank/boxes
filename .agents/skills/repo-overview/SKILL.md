...
- `.github/workflows/aur-bump.yml` runs audit → publish → prepare → review → finish on main via schedule/manual dispatch only. The threat model is **trust the upstream developers, distrust the AUR packagers**: everything a packager controls is text in the diff, including scriptlets that run as root at image build time and files sourced into every shell, so reviewing that text is the whole control. `scripts/validate-aur-diff.py` sorts each change into tier A (literal version/checksum updates moving forward — merged on a grammar check), tier B (bounded packager-authored recipe code — reviewed line by line with the diff *and* the whole of every changed file, then merged on PASS), or tier C (human only: deletions, renames, mode/type changes, binaries, new packages, control characters, over-cap changes, backwards versions, introduced `SKIP`s, non-literal pinning assignments, and any `source=`/`install=` change). `scripts/aur-review.py` publishes tier C as needs-review drafts without checking out or executing candidate files, and reclassifies independently at publish, prepare and finish. The model runs in an empty directory with safe mode, all built-in/MCP tools disabled and no GitHub credentials, using `CLAUDE_CODE_OAUTH_TOKEN` from the main-only `aur-review` environment. It receives only its tier's saved policy and payload, with untrusted sections fenced by the candidate's own commit hash. A separate finish job binds the verdict to the exact request/tier/policy — so a verdict cannot be replayed across tiers — rechecks publisher eligibility and GitHub Actions status binding, then performs a SHA-bound merge or posts escaped findings. After merging it explicitly dispatches `build.yml` because GITHUB_TOKEN does not trigger push workflows. Keep workflow, script, isolation tests and **both** policies (`.github/aur-vet-prompt.md`, `.github/aur-review-prompt.md`) synchronized. Policies come directly from the trusted workflow source; deployment and retirement of the old cloud routine are documented in `aur/README.md`. The pipeline scripts and tests are CI-only and excluded from image path filtering.
