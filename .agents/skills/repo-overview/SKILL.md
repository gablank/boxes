...
- `.github/workflows/aur-bump.yml` runs audit → publish → prepare → review → finish on main via schedule/manual dispatch only. `scripts/aur-review.py` publishes usable rejected changes as needs-review drafts without checking out or executing candidate files; only mechanically eligible candidates reach Claude. The model runs in an empty directory with safe mode, all built-in/MCP tools disabled and no GitHub credentials, using `CLAUDE_CODE_OAUTH_TOKEN` from the main-only `aur-review` environment. It receives only the saved policy and zero-context diff. A separate finish job binds the verdict to the exact request/policy, rechecks publisher eligibility and GitHub Actions status binding, then performs a SHA-bound merge or posts escaped findings. After merging it explicitly dispatches `build.yml` because GITHUB_TOKEN does not trigger push workflows. Keep workflow, script, isolation tests and `.github/aur-vet-prompt.md` synchronized. The policy comes directly from the trusted workflow source; deployment and retirement of the old cloud routine are documented in `aur/README.md`. The pipeline scripts and tests are CI-only and excluded from image path filtering.
