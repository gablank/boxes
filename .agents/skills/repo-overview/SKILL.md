...
- `.github/workflows/aur-bump.yml` runs audit → publish → review on main via schedule/manual dispatch only. `scripts/trigger-aur-review.py` checks publisher metadata before calling the Claude routine API with the main-only `aur-review` environment token. No PR event or PR text may reach this caller. Keep its payload, branch format and `.github/aur-vet-prompt.md` synchronized; deployment, permissions and recovery are documented in `aur/README.md`. Its offline security tests run in CI; both scripts are excluded from base-image path filtering.
